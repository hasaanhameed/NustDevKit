...
Source: /#/python/x-redirect/JTI0bSUyRk5vdGlmaWNhdGlvbg
...
| `subject` | `str` | Required | Subject line of the notification. |
| `shortenedsubject` | `str` | Required | Shortened version of the notification subject for compact display. |
| `text` | `str` | Required | Short HTML excerpt of the notification. |
| `fullmessage` | `str` | Required | Full plain-text body of the notification. |
| `fullmessageformat` | `int` | Required | Format identifier for fullmessagehtml (2 = Markdown source). |
| `fullmessagehtml` | `str` | Required | Full HTML body of the notification email. |
| `smallmessage` | `str` | Required | Short plain-text preview of the notification. |
| `contexturl` | `str` | Required | URL to the relevant context for this notification (e.g., a forum post). |
| `contexturlname` | `str` | Required | Human-readable label for the context URL link. |
| `timecreated` | `int` | Required | Unix timestamp when the notification was created. |
| `timecreatedpretty` | `str` | Required | Human-readable relative time string (e.g., '19 days ago'). |
...
| `iconurl` | `str` | Required | URL to the icon image representing the notification source. |
| `component` | `str` | Required | Moodle component that generated the notification (e.g., 'mod_forum'). |
| `eventtype` | `str` | Required | Event type within the component that triggered the notification (e.g., 'posts'). |
| `customdata` | `str` | Required | JSON-encoded string with additional notification metadata (cmid, instance, discussionid, postid, etc.). |
