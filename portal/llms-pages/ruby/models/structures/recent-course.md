...
Source: /#/ruby/x-redirect/JTI0bSUyRlJlY2VudENvdXJzZQ
...
| `summaryformat` | `Integer` | Required | Format of the summary field (1 = HTML). |
| `startdate` | `Integer` | Required | Course start date as Unix timestamp. |
| `enddate` | `Integer` | Required | Course end date as Unix timestamp. 0 means no end date. |
| `visible` | `TrueClass \| FalseClass` | Required | Whether the course is visible to enrolled students. |
...
| `progress` | `Integer` | Required | Student's completion progress as a percentage.<br><br>**Constraints**: `>= 0`, `<= 100` |
| `hasprogress` | `TrueClass \| FalseClass` | Required | Whether completion tracking is enabled for this course. |
| `isfavourite` | `TrueClass \| FalseClass` | Required | Whether the user has marked this course as a favourite. |
| `hidden` | `TrueClass \| FalseClass` | Required | Whether the user has hidden this course from their dashboard. |
| `showshortname` | `TrueClass \| FalseClass` | Required | Whether the short name is displayed alongside the full name. |
...
```ruby
recent_course = RecentCourse.new(
  id: 49900,
